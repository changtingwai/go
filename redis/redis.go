...
func main() {
	//设置输入参数的默认值及解析方法
	var host = flag.String("h", "192.168.222.40", "redis proxy")
	var port = flag.String("p", "7300", "redis port")
	var merge = flag.String("merge", "no", "merge insert value")
	//开始解析参数
	flag.Parse()
	fmt.Printf("conf info : host[%s], port[%s], merge[%s]\n", *host, *port, *merge)
	...
	for {
		...
		if len(cmdArr) != 3 {
			fmt.Println("input cmd lens err", cmdArr)
			continue
		}
		//fmt.Println(cmdArr)
			...
			if err == nil {
				...
				if merge_str != "" || cmdArr[2] == "" {
					...
					if merge_debug_num%merge_k == 0 {
						fmt.Println("get error num: ", get_err_num)
		...
		if err != nil {
			fmt.Println("执行redis命令错误.")
			fmt.Println("cmd:", cmd, "cmdArr:", cmdArr)
			panic(err)
		}
		line_num += 1
		if line_num%10000 == 0 {
