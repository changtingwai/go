...
func main() {
	...
	//Scan开始处理每一条数据
	for {
		cmd, err = bufio.readear.ReadString('\n')
		if err == io.EOF {
			break
		}
		...
		if len(cmdArr) != 3 {
			fmt.Fprintln(os.Stderr, "input cmd lens err", cmdArr)
			continue
		}
		//fmt.Fprintln(os.Stderr, cmdArr[1])
		//fmt.Println(cmdArr)
			...
			if err == nil {
				...
				if merge_str != "" || cmdArr[2] == "" {
					...
					if merge_debug_num%merge_k == 0 {
						fmt.Fprintln(os.Stderr, "get error num: ", get_err_num)
		...
		if err != nil {
			fmt.Fprintln(os.Stderr, "执行redis命令错误.")
			fmt.Fprintln(os.Stderr, "cmd:", cmd, "cmdArr:", cmdArr)
			panic(err)
		}
		line_num += 1
		if line_num%10000 == 0 {
